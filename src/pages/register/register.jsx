...
const Register = () => {
    ...
    const [fomrValues, setFormValues] = useState({name: '', email: '', password: '', repeatPassword: ''});
    const [fomrErrors, setFormErrors] = useState({});
    const [isOK, setOK] = useState({name: false, email: false, password: false, repeatPassword: false});
        ...
        let response = false
                    ...
                    .get('http://localhost:8080/api/v1/users/checkEmail?email=' + fomrValues.email ).then(res => {
                        ...
                        if(res.data.data === 'true'){
                            const formEtemp = {...fomrErrors};
                            formEtemp.email = "Email existed, please change";
                            setFormErrors(formEtemp);
                            console.log(fomrErrors)
                            isOK.email = false;
                            setOK(isOK);
                        }
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if(isOK.name && isOK.email && isOK.password && isOK.repeatPassword){
            axios
            .post(url,
                {
                    fullname: fomrValues.name,
                    email: fomrValues.email,
                    password: fomrValues.password
                } 
                ).then(res => {
                if(!res.data.status === 'success'){
                    setResponse('Fail to registration');
                }
                else {
                        ...
                        <div className="col-md-10 col-lg-6 col-xl-5 order-2 order-lg-1">
                            ...
                            <div className="d-flex flex-row align-items-center mb-4">
                                ...
                                <input type="text" id="form3Example1c" className="form-control" 
                                    value={fomrValues.name} onChange={handleOnchange} name="name"/>
