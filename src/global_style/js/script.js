    ...
    if ($("#slider-range").length) {
        ...
        marginSlider.noUiSlider.on('update', function (values, handle) {
            if (handle) {
                ...
                var res = str.split(".");
                marginMax.innerHTML = res[0] + " đ";
            } else {
                var str = values[handle]
                var res = str.split(".");
                marginMin.innerHTML = res[0] + " đ" + " - ";
            }
        });
    }
...
$(document).ready(function () {
    ...
    $(".pix-text").mousedown(function () {
        $(this).css("border", "1px solid #b1abab")
        $(".text-alert").css("display", "none")
    });
    $("#name").blur(function () {
        var inputTextName = $("#name").val();
        if (inputTextName.length == 0) {
            $("#name").css("border", "1px solid red")
            $(".alert-missing-name").css("display", "block")
        }
    });
    $("#uname").blur(function () {
        var inputTextUname = $("#uname").val();
        if (inputTextUname.length == 0) {
            $("#uname").css("border", "1px solid red")
            $(".alert-missing-uname").css("display", "block")
        }
    });
    $("#uname2").blur(function () {
        var inputTextUname2 = $("#uname2").val();
        if (inputTextUname2.length == 0) {
            $("#uname2").css("border", "1px solid red")
            $(".alert-missing-uname2").css("display", "block")
        }
    });
    $("#pass").blur(function () {
        var inputTextPass = $("#pass").val();
        if (inputTextPass.length == 0) {
            $("#pass").css("border", "1px solid red")
            $(".alert-missing-pass").css("display", "block")
        }
    });
    $("#email").blur(function () {
        var inputTextEmail = $("#email").val();
        if (inputTextEmail.length == 0) {
            $("#email").css("border", "1px solid red")
            $(".alert-missing-email").css("display", "flex")
        } else {
            if (inputTextEmail.search("@") < 0) {
                $("#email").css("border", "1px solid red")
                $(".alert-missing-email2").css("display", "flex")
            }
            if (inputTextEmail.search("@") == inputTextEmail.length - 1) {
                $("#email").css("border", "1px solid red")
                $(".alert-email-2").css("display", "flex")
            }
        }
    });
    $("#date").blur(function () {
        var inputTextCode = $("#date").val();
        if (inputTextCode.length == 0) {
            $("#date").css("border", "1px solid red")
            $(".alert-missing-date").css("display", "flex")
        }
    });
    $("#fo_pass").blur(function () {
        var inputTextFo_pass = $("#fo_pass").val();
        if (inputTextFo_pass.length == 0) {
            $("#fo_pass").css("border", "1px solid red")
            $(".alert-missing-fo_pass").css("display", "flex")
        }
    });
});